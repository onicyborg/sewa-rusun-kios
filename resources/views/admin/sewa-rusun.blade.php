...
@section('title')
    Manage Penyewaan Rusun - Admin
@endsection

@section('content')
    <div class="d-flex align-items-left align-items-md-center flex-column flex-md-row pt-2 pb-4">
        <div>
            <h3 class="fw-bold mb-3">Kelola Penyewaan Rusun</h3>
        </div>
            ...
            <div class="card card-round">
                <div class="card-header">
                    <div class="card-head-row">
                        <div class="card-title">Data Penyewaan Rusun</div>
                    </div>
                </div>
                <div class="card-body">
                    <div class="table-responsive">
                        <table id="PenyewaanRusunTable" class="display table table-striped table-hover" style="width:100%">
                            <thead>
                                <tr>
                                    <th>No</th>
                                    <th>Nama Penyewa</th>
                                                ...
                                                <div class="modal-body">
                                                    <div class="row">
                                                        ...
                                                        <div class="col-md-4">
                                                            <h5>Detail Rusun</h5>
                                                            <div>
                                                                <p><strong>Nomor Rusun:</strong>
                                                                    {{ $sewa->rusun->nomor_rusun }}</p>
                                                                <p><strong>Lantai:</strong> {{ $sewa->rusun->lantai }}</p>
                                                                <p><strong>Tower:</strong> {{ $sewa->rusun->tower }}</p>
                                                                <p><strong>Harga Sewa:</strong>
                                                                    Rp{{ number_format($sewa->rusun->harga_sewa, 0, ',', '.') }}
                                                                </p>
                                                            </div>
                                                        </div>
                                                    </div>
    ...
    <div class="modal fade" id="createModal" aria-labelledby="createModalLabel" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="createModalLabel">Tambah Penyewaan Rusun</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body">
                    <form id="createRusunForm" action="{{ route('sewa-rusun.store') }}" method="POST">
                        @csrf
                        ...
                        <!-- Field untuk Memilih Rusun -->
                        <div class="mb-3">
                            <label for="rusun" class="form-label">Rusun</label>
                            <select id="rusun" name="rusun"
                                class="form-control @error('rusun') is-invalid @enderror" style="width: 100%;">
                                <option value="" disabled {{ old('rusun') == '' ? 'selected' : '' }}>Pilih Rusun
                                </option>
                                @foreach ($rusuns as $rusun)
                                    <option value="{{ $rusun->id }}"
                                        {{ old('rusun') == $rusun->id ? 'selected' : '' }}>
                                        {{ $rusun->nomor_rusun }}
                                    </option>
                                @endforeach
                            </select>
    ...
    <div class="modal fade" id="addTagihanModal" tabindex="-1" aria-labelledby="addTagihanModalLabel"
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                <form id="addTagihanForm" method="POST" action="/tagihan-rusun/add">
                    @csrf
                    <div class="modal-header">
...
@push('scripts')
    ...
    <script>
        $(document).ready(function() {
            ...
            $('#PenyewaanRusunTable').DataTable();
            var table = $('#TagihanRusunTable').DataTable({
                processing: true,
                serverSide: true,
                ajax: {
                    url: "{{ route('tagihanRusun.index') }}", // URL untuk mengambil data dari server
                    data: function(d) {
                ...
                columns: [{
                    ...
                    },
                    {
                        ...
                        <div style="text-align: center;">
                            <a href="/detail-tagihan-bulanan/${row.bulan}/${row.tahun}" class="btn btn-sm btn-primary">
            ...
            document.getElementById('tanggal_mulai').addEventListener('change', function() {
                ...
                if (!isNaN(year) && !isNaN(month)) {
                    ...
                    tanggalMulai.setMonth(tanggalMulai.getMonth() + 24);
