...
@section('title')
    Detail Tagihan Rusun Bulan {{ $bulan . ' ' . $tahun }} - Admin
@endsection
...
@section('content')
    <div class="d-flex align-items-left align-items-md-center flex-column flex-md-row pt-2 pb-4">
        <div>
            <h3 class="fw-bold mb-3">List Tagihan Rusun Bulan {{ $namaBulan[$bulan] . ' ' . $tahun }}
                            ...
                            <tbody>
                                @foreach ($tagihans as $no => $tagihan)
                                    <tr data-sewa="{{ $tagihan->sewa }}" data-denda="{{ $tagihan->denda }}"
                                        ...
                                        <td>{{ $no + 1 }}</td>
                                        <td>{{ $tagihan->sewa_rusun->penyewa->name }}</td>
                                        ...
                                        <div class="modal-dialog modal-lg">
                                            <div class="modal-content">
                                                <form id="updateTagihanForm" method="POST"
                                                    action="{{ route('update.tagihan', $tagihan->id) }}">
                                                    @csrf
                                                    @method('PUT')
                                                    ...
                                                    <div class="modal-body">
                                                            ...
                                                            <div class="col-md-6">
                                                                ...
                                                                <ul class="list-group">
                                                                    <li class="list-group-item"><strong>Nama
                                                                            Penghuni:</strong>
                                                                        <span
                                                                            id="namaPenghuni">&nbsp;{{ $tagihan->sewa_rusun->penyewa->name }}</span>
                                                                    </li>
                                                                    <li class="list-group-item"><strong>No KTP:</strong>
                                                                        <span
                                                                            id="ktpPenghuni">&nbsp;{{ $tagihan->sewa_rusun->penyewa->nik }}</span>
                                                                    </li>
                                                                    <li class="list-group-item"><strong>Kontak:</strong>
                                                                        <span
                                                                            id="kontakPenghuni">&nbsp;{{ $tagihan->sewa_rusun->penyewa->whatsapp }}</span>
                                                                    </li>
                                                                </ul>
                                                            </div>
                                                            <div class="col-md-6">
                                                                <h6>Detail Rusun</h6>
                                                                <ul class="list-group">
                                                                    <li class="list-group-item"><strong>Tower:</strong>
                                                                        <span
                                                                            id="towerRusun">&nbsp;{{ $tagihan->sewa_rusun->rusun->tower }}</span>
                                                                    </li>
                                                                    <li class="list-group-item"><strong>Lantai:</strong>
                                                                        <span
                                                                            id="lantaiRusun">&nbsp;{{ $tagihan->sewa_rusun->rusun->lantai }}</span>
                                                                    </li>
                                                                    <li class="list-group-item"><strong>No. Rusun:</strong>
                                                                        <span
                                                                            id="nomorRusun">&nbsp;{{ $tagihan->sewa_rusun->rusun->nomor_rusun }}</span>
                                                                    </li>
                                                                </ul>
                                                            </div>
                                                        </div>
                                                        ...
                                                        <div class="mb-3">
                                                            ...
                                                            <input type="number" class="form-control" id="denda"
                                                                name="denda" value="{{ $tagihan->denda }}"
                                                                {{ $status == 'Release' ? 'readonly' : '' }}>
                                                        </div>
                                                        <div class="mb-3">
                                                            <label for="air" class="form-label">Biaya Air</label>
                                                            <input type="number" class="form-control" id="air"
                                                                name="air" value="{{ $tagihan->air }}"
                                                                {{ $status == 'Release' ? 'readonly' : '' }}>
                                                        </div>
                                                        <hr>
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-footer">
                    ...
                    <form action="{{ route('release.tagihan', ['bulan' => $bulan, 'tahun' => $tahun]) }}" method="POST"
...
@push('scripts')
    ...
    <script>
        $(document).ready(function() {
            const table = $('#PenyewaanRusunTable').DataTable();

            // Event listener untuk tombol toggle details
            $('#PenyewaanRusunTable').on('click', '.toggle-details', function() {
